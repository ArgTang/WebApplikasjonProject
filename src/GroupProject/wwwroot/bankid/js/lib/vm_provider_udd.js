...
define( ["." + "/lib/vm_provider", "utils", "ko", "locale"], function ( vm_provider, utils, ko, locale ) {
     ...
     * @constructor
     */
    vm_provider.BankIdViewModel = function () {
        var self = this;
        ...
        self.currentPeronalCertificate = ko.computed( function () {
            if ( self.current() ) {
                return {text: self.current().certificate};
            } else {
                return {text: ''};
            }
        } );
        ...
        self.addObject = function ( obj ) {
            ...
            utils.each( obj.histories, function ( history ) {
                ...
                else if ( history.error in locale ) {
                    history.error = locale[history.error];
                }
            } );
     ...
     * @constructor
     */
    vm_provider.Certificate = function ( certificate ) {
        var self = this;
